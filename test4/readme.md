...
本次实验所用用户为**wangwei_user**,实验中完整的SQL代码见文章末
...
1. 查询某个员工的信息
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询1.png)
...
3. 查询订单表，并且包括订单的订单应收货款: Trade_Receivable= sum(订单详表.ProductNum*订单详单表.ProductPrice)- Discount。
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询3.png)
...
4. 查询订单详表，要求显示订单的客户名称和客户电话，产品类型用汉字描述。
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询4.png)
...
5. 查询出所有空订单，即没有订单详单的订单。
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询5.png)
...
6. 查询部门表，同时显示部门的负责人姓名。
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询6.png)
...
7. 查询部门表，统计每个部门的销售总金额。
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查询7.png)
...
1. 查看数据文件的使用情况
   ...
   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查看数据文件的使用情况.png)

2. 查看表空间的使用情况

   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查看表空间的使用情况.png)

3. 查看数据文件大小

   ![](https://raw.githubusercontent.com/GoToThePast/oracle/master/test4/查看数据文件大小.png)

## 六、总结

通过本次实验，我了解了Oracle表和视图的概念，Oracle 数据库数据对象中最基本的是表和视图，其他还有约束、序列、函数、存储过程、包、触发器等。对数据库的操作可以基本归结为对数据对象的操作,理解和掌握 Oracle数据库对象是学习Oracle的捷径。

本次实验还让我学会了使用SQL语句Create Table创建表，学会了Select语句插入，修改，删除以及查询数据、使用SQL语句创建视图，并且学会了部分存储过程和触发器的使用。

## 七、本次实验完整的SQL语句

```sql
-- QUOTAS
ALTER USER wangwei_user QUOTA UNLIMITED ON USERS;
ALTER USER wangwei_user QUOTA UNLIMITED ON USERS02;
ALTER USER wangwei_user ACCOUNT UNLOCK;

-- ROLES
GRANT "CONNECT" TO wangwei_user WITH ADMIN OPTION;
GRANT "RESOURCE" TO wangwei_user WITH ADMIN OPTION;
ALTER USER wangwei_user DEFAULT ROLE "CONNECT","RESOURCE";

-- SYSTEM PRIVILEGES
GRANT CREATE VIEW TO wangwei_user WITH ADMIN OPTION;
...
--批量插入1万订单数据，注意ORDERS.TRADE_RECEIVABLE（订单应收款）的自动计算,注意插入数据的速度
declare
  dt date;
  m number(8,2);
